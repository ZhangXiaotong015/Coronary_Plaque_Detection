...
class RPNHead(nn.Module):
    def __init__(self, in_channels, num_anchors):
        ...
        # self.cls_logits = nn.Conv2d(in_channels, 2, 1)
        # self.bbox_pred = nn.Conv2d(in_channels, 2 * 2, 1)
        # self.mask = nn.Conv2d(in_channels, in_channels*72*2, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        # mask = self.mask(x)
        # mask = torch.reshape(mask, (x.shape[0], x.shape[-1]*x.shape[-2]*self.num_anchors, x.shape[1], x.shape[-2], x.shape[-1])) # B*N*C*H*W
...
class RegionProposalNetwork(nn.Module):
    def __init__(self, anchor_generator, head, 
        ...
        self.min_angle = 0#10 # deg

        # self.adaptive = AdaptiveLossFunction(num_dims=2, float_dtype=torch.float32, device = "cuda:0")
    ...
    def compute_loss(self, objectness, pred_bbox_delta, gt_box, anchor):
        ...
        'using adaptive loss to regress bbox coordinate'
        # box_loss =  torch.sum(self.adaptive.lossfun(torch.squeeze(pred_bbox_delta_box) - torch.squeeze(regression_target_box))) / (idx[0][1].numel()*len(idx))
    ...
    def forward(self, feature, image_shape, target=None):
        if target is not None:
            gt_box = target['bbox']
        anchor = self.anchor_generator(feature, image_shape) # batch_size*72*4
        
        objectness, pred_bbox_delta = self.head(feature) # objectness: batchSize*num_anchors*2*12  pred_bbox_delta: batchSize*2num_anchors*2*12  two parameters needed to be regressed
        objectness = objectness.permute(0, 2, 3, 1).flatten(start_dim=1) # batchSize*72
        # pred_bbox_delta = pred_bbox_delta.permute(0, 2, 3, 1).reshape(-1, 2) # for batch_size=1 only
        pred_bbox_delta = torch.cat([torch.unsqueeze(pred_bbox_delta[i,:].permute(1,2,0).reshape(-1,2),dim=0) for i in range(objectness.shape[0])]) # batchSize*72*2

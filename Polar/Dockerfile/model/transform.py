...
class Transformer:
    def __init__(self, min_size=None, max_size=None, image_mean=None, image_std=None):
...
def paste_masks_in_image(mask, box, padding, image_shape):
    ...
    for i in range(len(mask)):
        ...
        im_mask_ele = torch.zeros(size, dtype=mask[i].dtype, device=mask[i].device)
        im_mask.append(im_mask_ele)

    for m, b, im in zip(mask, box, im_mask):
        b = b.tolist()
        # w = max(b[2] - b[0], 1)
        # h = max(b[3] - b[1], 1)
        angle_interval = [max(sample[-2], 10 ) for sample in b]
        radius = 45
        
        # m = F.interpolate(m[None, None], size=(h, w), mode='bilinear', align_corners=False)[0][0]
        m = [F.interpolate(m[i][None, None], size=(radius, int(angle_interval[i])), mode='bilinear', align_corners=False)[0][0] for i in range(m.shape[0])]

        # x1 = max(b[0], 0)
        # y1 = max(b[1], 0)
        # x2 = min(b[2], image_shape[1])
        # y2 = min(b[3], image_shape[0])
        start_angle = [int(max(b[i][-1], 0)) for i in range(len(b))]
        start_radius = 0
        end_angle = [int(min(b[i][-1]+b[i][-2]+1, image_shape[1]))  for i in range(len(b))]
        end_radius = 45

        # im[y1:y2, x1:x2] = m[(y1 - b[1]):(y2 - b[1]), (x1 - b[0]):(x2 - b[0])]
        for i in range(len(start_angle)):
            # im[i,start_radius:end_radius, start_angle[i]:end_angle[i]] = m[i][start_radius:end_radius, start_angle[i]-int(b[i][-1]):end_angle[i]-int(b[i][-1])]
            im[i,start_radius:end_radius, start_angle[i]:start_angle[i]+m[i].shape[1]] = m[i]
    return im_mask